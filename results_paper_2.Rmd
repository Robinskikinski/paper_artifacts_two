...
anova(with_re, without_re, test = "LRT")
...
logLik_model1 <- logLik(with_re)
logLik_model2 <- logLik(without_re)
...
```{r}
with_re <- lm(LogSuperposition ~ RealFTIViewDiffic + LogBagComplexity + FTI_In_Bag + Block, data = PerformanceAndRatingsImagesWide) 
...
```{r}
with_re <- lm(AlignmentArtifact ~ RealFTIViewDiffic + LogBagComplexity + FTI_In_Bag + Block, data = PerformanceAndRatingsImagesWide) 
...
```{r}
with_re <- lm(PlacementArtifact ~ RealFTIViewDiffic + LogBagComplexity + FTI_In_Bag + Block, data = PerformanceAndRatingsImagesWide) 
