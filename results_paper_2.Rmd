...
```{r}
with_block <- lm(ImageDifficulty ~ RealFTIViewDiffic + LogSuperposition + LogBagComplexity + AlignmentArtifact  + PlacementArtifact + FTI_In_Bag + Block , data = PerformanceAndRatingsImagesWide) 

without_block <- lm(ImageDifficulty ~ RealFTIViewDiffic + LogSuperposition + LogBagComplexity + AlignmentArtifact  + PlacementArtifact + FTI_In_Bag, data = PerformanceAndRatingsImagesWide) 

anova( without_block, with_block)
...
logLik_model1 <- logLik(with_block)
logLik_model2 <- logLik(without_block)

# Log-Likelihood-Ratio berechnen
LLR <- -2 * (logLik_model1 - logLik_model2)
LLR
...
```{r}
with_block <- lm(LogSuperposition ~ RealFTIViewDiffic + LogBagComplexity + FTI_In_Bag + Block, data = PerformanceAndRatingsImagesWide) 

without_block <- lm(LogSuperposition ~ RealFTIViewDiffic + LogBagComplexity + FTI_In_Bag, data = PerformanceAndRatingsImagesWide) 
...
```{r}
with_block <- lm(AlignmentArtifact ~ RealFTIViewDiffic + LogBagComplexity + FTI_In_Bag + Block, data = PerformanceAndRatingsImagesWide) 

without_block <- lm(AlignmentArtifact ~ RealFTIViewDiffic + LogBagComplexity + FTI_In_Bag, data = PerformanceAndRatingsImagesWide) 
...
```{r}
with_block <- lm(PlacementArtifact ~ RealFTIViewDiffic + LogBagComplexity + FTI_In_Bag + Block, data = PerformanceAndRatingsImagesWide) 

without_block <- lm(PlacementArtifact ~ RealFTIViewDiffic + LogBagComplexity + FTI_In_Bag, data = PerformanceAndRatingsImagesWide) 
...
lavInspect(results_model_paper_dummy_all, "rsquare")
```
...
```{r}
effect_table <- parameterEstimates(results_model_paper_dummy_all) %>%
  as.tibble() %>%
  filter(op == "~") %>%
    mutate(
    effect = paste0(rhs, " -> ", lhs)
  ) %>%
  select(effect, est)

temp <- standardizedSolution(results_model_paper_dummy_all) %>%
  as.tibble() %>%
  mutate(
    effect = paste0(rhs, " -> ", lhs)
  ) %>%
  select(effect,rhs,lhs,  est.std, ci.lower, ci.upper, pvalue)

effect_table <- effect_table %>%
  left_join(temp, by = "effect") %>%
  mutate_at(
    .vars = c(
      "est",
      "est.std",
...
effect_table <- effect_table %>%
  mutate(
    Type = case_when(
      effect == "PlacementArtifact -> ImageDifficulty" ~ "H1a",
      effect == "AlignmentArtifact -> ImageDifficulty" ~ "H1b",
      effect == "RealFTIViewDiffic -> ImageDifficulty" ~ "H2a",
      effect == "LogSuperposition -> ImageDifficulty" ~ "H2b",
      effect == "LogBagComplexity -> ImageDifficulty" ~ "H2c",
      effect == "LogBagComplexity -> LogSuperposition" ~ "H2d",
      effect == "LogSuperposition -> PlacementArtifact" ~ "I1",
      effect == "LogBagComplexity -> PlacementArtifact" ~ "I2",
      effect == "RealFTIViewDiffic -> AlignmentArtifact" ~ "I3",
      effect == "LogBagComplexity -> AlignmentArtifact" ~ "I4",
      effect == "RealFTIViewDiffic -> LogSuperposition" ~ "I5",
      effect == "FTI_In_Bag -> PlacementArtifact" ~ "I6",
      effect == "FTI_In_Bag -> AlignmentArtifact" ~ "I7",
      effect == "FTI_In_Bag -> ImageDifficulty" ~ "I8",
      effect == "FTI_In_Bag -> LogSuperposition" ~ "I9",
      TRUE ~ "Rest"
    )
  ) %>%
  arrange(Type, lhs, rhs) %>%
  mutate(
    CI_95 = paste0("[", ci.lower, ",", ci.upper, "]")
  )


effect_table %>%
  select(Type, effect, est, est.std, CI_95, pvalue) %>%
  kable() %>%
  kable_styling()
